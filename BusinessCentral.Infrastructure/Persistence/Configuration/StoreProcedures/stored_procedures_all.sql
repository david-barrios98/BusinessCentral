...
CREATE OR ALTER PROCEDURE [construction].[sp_create_construction_project]
(
    @CompanyId INT,
    @Name VARCHAR(200),
...
BEGIN
    ...
    INSERT INTO [construction].[ConstructionProject]
    (CompanyId, Name, Description, Location, StartDate, EndDate, Budget, Active, CreatedAt, UpdatedAt)
    VALUES (@CompanyId, @Name, @Description, @Location, @StartDate, @EndDate, @Budget, 1, SYSUTCDATETIME(), SYSUTCDATETIME());

    SELECT CAST(SCOPE_IDENTITY() AS INT) AS InsertedId;
END
GO

CREATE OR ALTER PROCEDURE [construction].[sp_get_construction_project]
(
    @ProjectId INT
)
AS
BEGIN
    ...
    SELECT
        ...
        CreatedAt,
        UpdatedAt
    FROM [construction].[ConstructionProject]
    WHERE Id = @ProjectId;
END
GO
CREATE OR ALTER PROCEDURE [construction].[sp_update_construction_project]
(
    @ProjectId INT,
    @Name VARCHAR(200),
    @Description VARCHAR(1000),
...
BEGIN
    ...
    UPDATE [construction].[ConstructionProject]
    SET Name = @Name,
        Description = @Description,
        Location = @Location,
        StartDate = @StartDate,
...
END
GO
CREATE OR ALTER PROCEDURE [construction].[sp_delete_construction_project]
(
    @ProjectId INT
)
AS
...
END
GO
CREATE OR ALTER PROCEDURE [construction].[sp_list_construction_projects]
(
    @CompanyId INT,
    @Page INT,
    @PageSize INT
...
GO
-- 1) APU Insert
CREATE OR ALTER PROCEDURE [construction].[sp_insert_apu_item]
(
    @ProjectId INT,
    @Code VARCHAR(100),
    @Description VARCHAR(1000),
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ApuItem] (ProjectId, Code, Description, Unit, UnitPrice, Yield, CreatedAt)
    VALUES (@ProjectId,@Code,@Description,@Unit,@UnitPrice,@Yield,SYSUTCDATETIME());
    SELECT CAST(SCOPE_IDENTITY() AS INT) AS InsertedId;
END
GO

-- 2) WorkLog & Photos
CREATE OR ALTER PROCEDURE [construction].[sp_insert_worklog]
(
    @ProjectId INT,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[WorkLog] (ProjectId, LoggedByUserId, LogDate, Notes, CreatedAt)
    VALUES (@ProjectId, @LoggedByUserId, ISNULL(@LogDate, SYSUTCDATETIME()), @Notes, SYSUTCDATETIME());
    SELECT CAST(SCOPE_IDENTITY() AS INT) AS InsertedId;
END
GO

CREATE OR ALTER PROCEDURE [construction].[sp_insert_worklog_photo]
(
    @WorkLogId INT,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[WorkLogPhoto] (WorkLogId, FilePath, FileName, CreatedAt)
    VALUES (@WorkLogId, @FilePath, @FileName, SYSUTCDATETIME());
END
GO

-- 3) Progress
CREATE OR ALTER PROCEDURE [construction].[sp_upsert_progress_item]
(
    @ProjectId INT,
    @Title VARCHAR(500),
    @Percentage INT,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ProgressItem] (ProjectId, Title, Percentage, Notes, UpdatedAt)
    VALUES (@ProjectId,@Title,@Percentage,@Notes,SYSUTCDATETIME());
END
GO

-- 4) Change Orders
CREATE OR ALTER PROCEDURE [construction].[sp_create_change_order]
(
    @ProjectId INT,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ChangeOrder] (ProjectId, Description, Amount, RequestedByUserId, Status, CreatedAt)
    VALUES (@ProjectId,@Description,@Amount,@RequestedByUserId,'PENDING',SYSUTCDATETIME());
    SELECT CAST(SCOPE_IDENTITY() AS INT) AS InsertedId;
END
GO

-- 5) Tool Loans
CREATE OR ALTER PROCEDURE [construction].[sp_loan_tool]
(
    @ToolId INT,
    @ProjectId INT = NULL,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ToolLoan] (ToolId, ProjectId, BorrowedByUserId, LoanDate, Notes)
    VALUES (@ToolId,@ProjectId,@BorrowedByUserId,SYSUTCDATETIME(),@Notes);
END
GO

-- 6) Project Expenses
CREATE OR ALTER PROCEDURE [construction].[sp_insert_project_expense]
(
    @ProjectId INT,
    @Amount DECIMAL(18,2),
    @Concept VARCHAR(500),
    @SpentByUserId INT = NULL
)
AS
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ProjectExpense] (ProjectId, Amount, Concept, SpentByUserId, CreatedAt)
    VALUES (@ProjectId,@Amount,@Concept,@SpentByUserId,SYSUTCDATETIME());
    SELECT CAST(SCOPE_IDENTITY() AS INT) AS InsertedId;
END
GO

-- 7) Attendance
CREATE OR ALTER PROCEDURE [construction].[sp_insert_attendance]
(
    @ProjectId INT,
    @UserId INT,
    @DateWorked DATE,
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[Attendance] (ProjectId, UserId, DateWorked, Hours, CreatedAt)
    VALUES (@ProjectId,@UserId,@DateWorked,@Hours,SYSUTCDATETIME());
END
GO

-- 8) Project Documents
CREATE OR ALTER PROCEDURE [construction].[sp_insert_project_document]
(
    @ProjectId INT,
    @FilePath VARCHAR(1000),
    @FileName VARCHAR(255),
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [construction].[ProjectDocument] (ProjectId, FilePath, FileName, DocumentType, UploadedByUserId, UploadedAt)
    VALUES (@ProjectId,@FilePath,@FileName,@DocumentType,@UploadedByUserId,SYSUTCDATETIME());
END
GO